    ...
    for prediction_type in input_data.prediction_types:
        ...
        if prediction_type == "bioactivity_ic50" and input_data.target:
            try:
                ...
                logger.info(f"🧠 Using True GNN model for {input_data.target} IC50 prediction")
...
try:
    ...
    try:
        ...
        logger.info("🧠 Simple GNN predictor loaded as fallback")
        ...
        try:
            ...
            logger.warning(f"Could not initialize any real predictor: {e3}")
            real_predictor = None
