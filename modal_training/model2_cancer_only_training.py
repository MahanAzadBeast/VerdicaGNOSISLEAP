"""
Model 2 CANCER-ONLY TRAINING - Simplified focus on high cancer R²
CHANGES:
1. REMOVE normal cells completely - focus only on cancer IC50
2. Target >0.6 R² for cancer IC50
3. Save checkpoints after EVERY epoch
4. Simplified architecture without joint prediction complexity
"""
...
# Optimized environment for stable training
image = (
...
app = modal.App("model2-cancer-only")
...
def train_cancer_only_model():
    ...
    print("🧬 GNOSIS MODEL 2: CANCER-ONLY TRAINING")
    print("=" * 80)
    
    # Initialize wandb
    wandb.init(
        project="gnosis-model2-cancer-only",
        name=f"cancer-only-training-{datetime.now().strftime('%Y%m%d-%H%M%S')}",
        config={
    ...
    print("📊 STEP 1: Loading GDSC cancer data...")
    ...
    total_steps = len(train_df) // 32 * 50  # batch_size=32, epochs=50
    ...
    print("   ✅ Scheduler: Cosine annealing with 10% warmup")
    ...
    print(f"\n✅ CANCER-ONLY IMPROVEMENTS:")
    print("  • 🎯 Simplified architecture (no normal cells)")
    print("  • 💾 Epoch-by-epoch checkpointing")
    print("  • 🧬 Pure cancer IC50 focus")
    ...
    return {
        ...
        'checkpoints_saved': num_epochs + 1  # Including best model
    }
