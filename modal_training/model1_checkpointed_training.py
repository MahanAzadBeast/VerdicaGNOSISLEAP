"""
Model 1 IMPROVED WITH CHECKPOINTING - Restart with epoch-by-epoch saves
CHANGES:
1. Add epoch-by-epoch checkpointing
2. Use --detach flag to prevent disconnection
3. Same excellent architecture that achieved R² = 0.5994
4. Target >0.6 R² with full 50 epochs
"""
...
# Proven environment setup that worked before
image = (
...
app = modal.App("model1-checkpointed")
...
def train_model1_with_checkpointing():
    ...
    print("🧬 GNOSIS MODEL 1: CHECKPOINTED TRAINING")
    print("=" * 80)
    
    # Initialize wandb
    wandb.init(
        project="gnosis-model1-checkpointed",
        name=f"model1-checkpointed-{datetime.now().strftime('%Y%m%d-%H%M%S')}",
        config={
    ...
    print("📊 STEP 1: Loading Model 1 training data...")
