...
async def predict_with_real_chemprop(input_data: SMILESInput):
    ...
    try:
        ...
        # Make prediction using direct app import
        logger.info(f"Making real Chemprop prediction for: {input_data.smiles}")
        
        # Import the app for context
        from chemprop_production_inference import app
        
        # Run prediction with proper app context
        with app.run():
            prediction_result = predict_function.remote(input_data.smiles)
        ...
        if prediction_result.get("status") == "success":
            ...
            return ChempropPredictionResponse(
                ...
                model_info={
                    ...
                    "architecture": "5-layer Message Passing Neural Network",
                    "training_method": "Multi-task GNN regression (REAL model)",
                    "real_model": True,
                    "system_type": "real_trained_chemprop_50epoch",
                    "training_epochs": 50
                },
